...
Vagrant::configure(VAGRANTFILE_API_VERSION) do |global_config|
  ...
  global_config.vm.define("ldap1.internal.tld") do |config|
    config.hostmanager.enabled = true
    config.hostmanager.manage_host = true
    config.hostmanager.ignore_private_ip = false
    config.hostmanager.include_offline = true
    config.vm.network :private_network, ip: "10.0.1.201"
    config.vm.hostname = "ldap1.internal.tld"
    ...
    config.vm.provider :virtualbox do |vb|
      ...
      vb.customize [ "createhd", "--filename", "disk-ldap1", "--size", "5000" ]
      vb.customize [ "storageattach", :id, "--storagectl", "SATAController", "--port", 4, "--device", 0, "--type", "hdd", "--medium", "disk-ldap1.vdi" ]
    end
   end
   
   global_config.vm.define("ldap2.internal.tld") do |config|
    config.hostmanager.enabled = true
    config.hostmanager.manage_host = true
    config.hostmanager.ignore_private_ip = false
    config.hostmanager.include_offline = true
    config.vm.network :private_network, ip: "10.0.1.202"
    config.vm.hostname = "ldap2.internal.tld"
    config.vm.box_url = "https://vagrantcloud.com/ubuntu/boxes/trusty64/versions/14.04/providers/virtualbox.box"
    config.vm.box = "ubuntu/trusty64"
    ...
    config.vm.provider :virtualbox do |vb|
      ...
      vb.customize [ "createhd", "--filename", "disk-ldap2", "--size", "5000" ]
      vb.customize [ "storageattach", :id, "--storagectl", "SATAController", "--port", 4, "--device", 0, "--type", "hdd", "--medium", "disk-ldap2.vdi" ]
    end
